...
Shadowed by a barbarous history of racial policing and state-sanctioned violence, the American opioid epidemic has been a crucible for relentless mass death and blistering chasms of racial inequity. Opioids are a broad class of strong painkillers which include medically prescribed drugs, heroin, and chemically synthesized opioids; examples include Morphine, Vicodin, OxyContin, Percocet, and Fentanyl. Since the late 1990s, opioid abuse rates have been sharply increasing, and it wasn't classified as a national public health emergency until 2017, with the relative popularity of specific opioid classes varying over time and place; [experts have generally broken the epidemic into three epochs or waves:]((https://www.cdc.gov/drugoverdose/epidemic/index.html)) [CITE @cdc_2021]:
...
> [We knew we couldn't make it illegal to be either against the war or black, but by getting the public to associate the hippies with marijuana and blacks with heroin, and then criminalizing both heavily, we could disrupt those communities. We could arrest their leaders, raid their homes, break up their meetings, and vilify them night after night on the evening news. Did we know we were lying about the drugs? Of course we did.](https://www.govinfo.gov/content/pkg/BILLS-115hres933ih/html/BILLS-115hres933ih.htm) [CITE @law_2018]

These specifically targeted attacks have had longstanding effects on the racial dimensions of the opioid epidemic. Namely, the Opioid epidemic is considered a predominantly "White" public health issue [as White people accounted for 70-85% of all opioid overdoses between 1999 and 2016.](https://www.kff.org/other/state-indicator/opioid-overdose-deaths-by-raceethnicity/?dataView=1&currentTimeframe=20&selectedRows=%7B%22wrapups%22:%7B%22united-states%22:%7B%7D%7D%7D&sortModel=%7B%22colId%22:%22Location%22,%22sort%22:%22asc%22%7D) The dramatic racial disparities in opioid overdose is widely considered to be a byproduct of the War on Drugs racialization of opioid abuse and its resultant influences on the [systematic non-prescription of painkillers to Black and Latinx communties](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0226490) While multiple analytic approaches have clarified the risk factors of opioid overdose, measuring opioid abuse is a generally more difficult task due to implicit negative survivorship biases (i.e. the people who die from overdose are not all the people using opioids) and sweeping variations in data infrastructure and reporting practices. While multiple analyses have presented sufficient evidence suggesting strong geographic and eco-social dimensions to opioid overdose, neighborhoods in Seattle, King County, Washington are relatively unique and insufficiently studied. The paucity of reports on Seattle is especially troubling when considering the high prevalence of opioid related deaths, accounting for 67% of all drug-related deaths between 2010-2017 ((Opioid Overdose Deaths Remain High in Seattle and King County) and claiming more lives than [car accidents— a leading cause of death nationwide— in 2017](https://www.atg.wa.gov/opioid-epidemic) [CITE @addiction_2022]. Further, due to the dramatic wealth disparities between economically disenfranchised neighborhoods and neighborhoods of workers at major commercial and tech companies, Seattle is a rich case study of neighborhood level geographies.
...
- **Latinx:**  The percent of residents identifying as Hispanic/Latinx in a defined census  tract.
  + To scale this coefficients to be practically significant, we multiplied this variable by 5.
- **Immigrant:** The percent of residents whoe immigrated to the United States living in a defined census  tract.
  + To scale this coefficients to be practically significant, we multiplied this variable by 5.
...
drug_map %>% ggplot()+ geom_density(size=1.5,aes(x=narc_count, color=factor_year)) + facet_wrap(.~factor_year, ncol=2,scale="free")+
  ...
  theme(legend.position="none",legend.title=element_blank(),
        ...
        strip.text.x=element_text(size=30, color="black", face="bold", family = 'DIN Condensed'),
...
We aimed to model narcotic arrest counts throughout Seattle, King County, Washington using 3 separate candidate models. Prior to statistical analysis, we plot the observed incidence rates (IRs) for all7 13 census tracts within the SPD patrol boundaries on the log scale.
...
Consistently, our worst tract in terms of predictions was 53033005301, a tract in the University District on the north western border of University of Washington. Mostly inhabited by rich college students, this tract has an average age of around 22, is predominantly White and Asian, and has a moderately high amount of narcotic arrests. The combination of a very unique demographic make up combined with its arrest numbers explains why our model tends to struggle with it.

One consideration is that we are inherently limited by the fact that we only have narcotic arrest data and not something like hospital data. Many systemic issues with the policing in America also plays a role in our data. Racial profiling, false arrests, and arrest quotas based on the war on drugs all play a part in decreasing how representative our model is of reality. It is also important to note that arrests are not the same as opioid abuse, and something like looking at cases of opioid use disorder would more accurately reflect the number of cases. However, due to the illegal nature of heroin and other opiates, as well as ethical issues with the disclosure of medical information, this is the best that we have to work with.
...
```{r, echo=FALSE}
drug_census_clean2<-drug_census_clean %>%
  group_by(GEOID) %>%
  mutate(black_change = black - black[Year==2010]) %>%
  mutate(narc_count_change = narc_count - narc_count[Year==2010]) %>%
  mutate(median_rent_change = median_rent - median_rent[Year==2010]) %>%
  mutate(immigrant_change = immigrant - immigrant[Year==2010])
```

```{r,echo=FALSE}
drug_map2 <- drug_census_clean2 %>%
  filter(Year !=2010) %>%
  mutate(Year = paste0("January 1, ", Year)) %>%
  mutate(Year = mdy(Year)) %>%
  mutate(Year = as_date(Year)) %>%
...
drug_map2 %>%
  ggplot() +
  geom_sf(aes(fill=black_change),  color = '#999999', size=.2)+
  geom_sf(data=roads_sub)+
  geom_sf(data=water_sub, color='#D6F3FF', fill='#D6F3FF') +
  scale_fill_gradientn(colors = c('#6B5CA5','#71A9F7','#FFFFFF', '#E75A7C', '#6C0E23'), na.value = NA)  +
  facet_wrap(.~factor_year, ncol=3) +
  theme(axis.text = element_blank(),
        axis.ticks = element_blank(),
        panel.grid.major = element_line('transparent'),
        panel.background = element_blank(),
      strip.text.x  = element_text(color='black', family='DIN Condensed', size = 20, face = 'bold', hjust=.5),
        ...
        legend.direction='horizontal',
        legend.text = element_text(color='black', size = 12, face='bold', family='DIN Condensed'))+
    guides(fill = guide_colourbar(title = 'Change in Percent (%) Black from 2010', 
                                  title.position='top', title.hjust =.5))

drug_map2 %>%
  ggplot() +
  geom_sf(aes(fill=immigrant_change),  color = '#999999', size=.2)+
  geom_sf(data=roads_sub)+
  geom_sf(data=water_sub, color='#D6F3FF', fill='#D6F3FF') +
  scale_fill_gradientn(colors = c('#6B5CA5','#71A9F7','#FFFFFF', '#E75A7C', '#6C0E23'), na.value = NA)  +
  facet_wrap(.~factor_year, ncol=) +
  theme(axis.text = element_blank(),
        axis.ticks = element_blank(),
        panel.grid.major = element_line('transparent'),
        panel.background = element_blank(),
      strip.text.x  = element_text(color='black', family='DIN Condensed', size = 20, face = 'bold', hjust=.5),
        ...
        legend.direction='horizontal',
        legend.text = element_text(color='black', size = 12, face='bold', family='DIN Condensed'))+
    guides(fill = guide_colourbar(title = 'Change in Percent (%) immigrant from 2010', 
                                  title.position='top', title.hjust =.5))

drug_map2 %>%
  ggplot() +
  geom_sf(aes(fill=median_rent_change/100000),  color = '#999999', size=.2)+
  geom_sf(data=roads_sub)+
  geom_sf(data=water_sub, color='#D6F3FF', fill='#D6F3FF') +
  scale_fill_gradientn(colors = c('#6B5CA5','#FFFFFF','#f2a6b8', '#E75A7C', '#c91d45', '#701026', '#430a17', '#000000'), na.value = NA)  +
  facet_wrap(.~factor_year, ncol=4) +
  theme(axis.text = element_blank(),
        axis.ticks = element_blank(),
        panel.grid.major = element_line('transparent'),
        panel.background = element_blank(),
      strip.text.x  = element_text(color='black', family='DIN Condensed', size = 20, face = 'bold', hjust=.5),
        ...
        legend.direction='horizontal',
        legend.text = element_text(color='black', size = 12, face='bold', family='DIN Condensed'))+
    guides(fill = guide_colourbar(title = 'Change in Median Rent($100,000) from 2010', 
                                  title.position='top', title.hjust =.5))

drug_map2 %>%
  ggplot() +
  geom_sf(aes(fill=narc_count_change),  color = '#999999', size=.2)+
  geom_sf(data=roads_sub)+
  geom_sf(data=water_sub, color='#D6F3FF', fill='#D6F3FF') +
  scale_fill_gradientn(colors = c('#6B5CA5','#71A9F7','#FFFFFF', '#E75A7C', '#6C0E23'), na.value = NA)  +
  facet_wrap(.~factor_year, ncol=4) +
  theme(axis.text = element_blank(),
      ...
      strip.text.x  = element_text(color='black', family='DIN Condensed', size = 20, face = 'bold', hjust=.5),
        ...
        legend.direction='horizontal',
        legend.text = element_text(color='black', size = 12, face='bold', family='DIN Condensed'))+
    guides(fill = guide_colourbar(title = 'Change in Narcotic Arrests from 2010 ', 
                                  title.position='top', title.hjust =.5))
```